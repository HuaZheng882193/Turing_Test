...
import { INITIAL_GREETINGS, TYPING_SPEED_MS_PER_CHAR_AI, TYPING_SPEED_MS_PER_CHAR_HUMAN } from './constants';
...
const App: React.FC = () => {
  ...
  const handleVote = (vote: EntityType) => {
    ...
    setStats(prev => ({
      ...
      aiEncounters: entityType === 'ai' ? prev.aiEncounters + 1 : prev.aiEncounters,
      humanEncounters: entityType === 'human' ? prev.humanEncounters + 1 : prev.humanEncounters
    }));
        ...
        <main className="flex-1 flex flex-col relative">
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-4 relative">
             ...
             {phase === 'connecting' && (
               <div className="h-full flex flex-col items-center justify-center space-y-4">
                  <div className="font-mono text-emerald-500 text-lg">正在搜索匹配对象...</div>
                  <div className="w-64 h-2 bg-emerald-900/30 rounded-full overflow-hidden">
                    <div className="h-full bg-emerald-500 animate-[loading_2s_ease-in-out_infinite]" style={{width: '30%'}}></div>
                  </div>
             ...
             {(phase === 'chatting' || phase === 'voting' || phase === 'reveal') && (
               <>
                 {messages.map((msg) => (
          ...
          {phase === 'voting' && (
            <div className="absolute inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-20 animate-in fade-in duration-300">
               <div className="bg-zinc-900 border border-emerald-500/30 p-8 rounded-lg max-w-lg w-full text-center shadow-2xl shadow-emerald-900/20">
                 <h2 className="text-2xl text-white font-bold mb-6 tracking-wider">请做出裁决</h2>
                 <p className="text-gray-400 mb-8">根据刚才的互动，识别该实体的身份。</p>
                 <div className="grid grid-cols-2 gap-4">
                   ...
                   >
                     ...
                     <span className="text-lg font-bold text-gray-200 group-hover:text-emerald-400">人类</span>
                   </button>
                   <button 
                   ...
                   >
                     ...
                     <span className="text-lg font-bold text-gray-200 group-hover:text-cyan-400">机器</span>
                   </button>
                 </div>
               </div>
            </div>
          ...
          <div className="p-4 border-t border-emerald-900/30 bg-black z-10">
            <div className="relative max-w-4xl mx-auto flex items-center gap-2">
              <span className="text-emerald-700 font-mono hidden md:block">{'>'}</span>
              <input 
                ref={inputRef}
                type="text" 
                value={inputValue}
                ...
                placeholder={phase === 'chatting' ? "输入消息..." : "系统空闲..."}
              ...
              <button 
                ...
                disabled={!inputValue.trim() || phase !== 'chatting' || isTyping}
